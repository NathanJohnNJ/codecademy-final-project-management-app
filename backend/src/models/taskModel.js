...
const Task = {
  ...
  async allUserTasks(user_id) {
    const res = await db.query('SELECT id, title, notes, priority, completed, due_date, created_date, project_id, team_id, user_ids FROM tasks WHERE $1=ANY(user_ids) ORDER BY due_date', [user_id]);
    return res.rows;
  },

  // Return all tasks belonging to team
  async allTeamTasks(team_id) {
    const res = await db.query('SELECT id, title, notes, priority, completed, due_date, created_date, project_id, team_id, user_ids FROM tasks WHERE team_id=$1 ORDER BY due_date', [team_id]);
    return res.rows;
  },

  // Return all tasks belonging to project
  async allProjectTasks(project_id) {
    const res = await db.query('SELECT id, title, notes, priority, completed, due_date, created_date, project_id, team_id, user_ids FROM tasks WHERE project_id=$1 ORDER BY due_date', [project_id]);
    return res.rows;
  },

  // Return task by id
  async findById(id) {
    const res = await db.query('SELECT id, title, notes, priority, completed, due_date, created_date, project_id, team_id, user_ids FROM tasks WHERE id=$1', [id]);
    return res.rows[0];
  },
